...
assert.equal((await me.puter.kv.get("archify_public_p1")).name, "Loft");
assert.equal((await me.puter.kv.get("archify_public_p1")).ownerId, "uuid-alice");
...
const pub = await me.puter.kv.get("archify_public_p1");
...
assert.equal(await me.puter.kv.get("archify_public_p1"), null);
assert.deepEqual(ids(await call("GET /api/projects/list", { user: alice })), ["p1"]);
...
// 6. legacy records written before ownerId existed are still owned by the
//    caller whose KV they live in, so list/get must normalise them
const carol = mkUser("uuid-carol", "carol");
await carol.puter.kv.set("roomify_project_legacy1", {
  id: "legacy1",
  name: "Old Loft",
  sourceImage: "https://x.puter.site/old.png",
  renderedImage: "https://x.puter.site/old-r.png",
  // no ownerId, as the previous save route wrote it
});

// the key above still uses the pre-rename prefix on purpose: reads must find it

const legacyList = await call("GET /api/projects/list", { user: carol });
assert.equal(legacyList.body.projects[0].ownerId, "uuid-carol");

const legacyGet = await call("GET /api/projects/get", {
  user: carol,
  query: "id=legacy1",
});
assert.equal(legacyGet.body.project.ownerId, "uuid-carol");

// and it can now actually be published, which the missing ownerId blocked
const legacyShared = await call("POST /api/projects/visibility", {
  user: carol,
  body: { id: "legacy1", visibility: "public" },
});
assert.equal(legacyShared.status, 200);
assert.equal(legacyShared.body.project.ownerId, "uuid-carol");

// normalising must never claim someone else's published record
assert.equal(
  (await call("GET /api/projects/get", { user: bob, query: "id=legacy1" }))
    .body.project.ownerId,
  "uuid-carol",
);

// 7. a record under the old prefix is readable, and publishing it writes the
//    public copy under the new one
assert.equal(await me.puter.kv.get("archify_public_legacy1") !== null, true);
assert.equal(await carol.puter.kv.get("roomify_project_legacy1") !== null, true);
